    ...
    if(scene){
        ...
        if(transform){
            let newTransform:any = JSON.parse(JSON.stringify(transform))
                ...
                newActionData.px = newTransform.p.x
                newActionData.py = newTransform.p.y
                newActionData.pz = newTransform.p.z
        }
    }
}
...
export function updateSetPositionEntity(direction:string, factor:number, manual?:boolean){
    ...
    newActionData.px = transform.x
    newActionData.py = transform.y
    newActionData.pz = transform.z
}
